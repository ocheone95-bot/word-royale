// Серверная копия игровой логики из @word-royale/shared.
// Дублирует scoring + daily-seed, потому что Deno-бандлер Edge Functions
// не любит .js-импорты, ссылающиеся на .ts-файлы (они есть внутри shared).
//
// При изменении логики в @word-royale/shared — синхронизировать с этим файлом.
// На Polish-неделе закроем snapshot-тестом эквивалентности.
...
// ===== Композируемость слова из набора букв =====
// Каждая буква в наборе может использоваться в слове ровно столько раз,
// сколько встречается в наборе (сейчас всегда 1, но логика общая).

export function isComposableFrom(word: string, letters: readonly string[]): boolean {
